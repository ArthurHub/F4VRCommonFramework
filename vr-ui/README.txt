...
4.1 If you change the width/height ration see "changing element size" below.


Advance: Changing element size
If changing the nif texture offset changed the proportions of the shape. i.e. the width to height ratio, you have to update the vortexes to reflect it and update the node name that contains the ratio value for the code to read it.
Note: only the proportions are important as the shape is always scaled, never change the height values ("Z").
1. Open in NifSkope
2. In Block List tree right click on the second element: "1 BSTriShape"
